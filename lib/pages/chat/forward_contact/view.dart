...
class ForwardContactPage extends GetView<ForwardContactController> {
  const ForwardContactPage({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return GetBuilder<ForwardContactController>(
      init: ForwardContactController(),
      id: "forward_contact",
      builder: (_) {
        return Scaffold(
          appBar: MyAppBar(
  ...
  Widget _buildView() {
    var showItems = <Widget>[
      ...
      ListTile(
          title: Text('选择群聊', style: TextStyle(fontSize: 18)),
          trailing: Icon(Icons.keyboard_arrow_right_outlined),
          onTap: controller.onTapGroup),
      Divider(
        height: 1,
      ),
    ];
